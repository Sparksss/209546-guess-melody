...
describe(`Check user points`, () => {
  it(`should return -1 if answers less 10`, () => {
    ...
    assert.equal(-1, getResults(new Array(5).fill({answer: true, timeLimit: 40}), 1), `quantity of answers less ten`);
  ...
  it(`should return 10 if answers is true but time greater than 30 seconds for one question`, () => {
    assert.equal(10, getResults(new Array(10).fill({answer: true, timeLimit: 40}), 1));
  });
